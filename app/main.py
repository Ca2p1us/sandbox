...
if choice == "2":
    ...
    log_fitness_histories(int(evaluate_num), "_"+str(NUM_GENERATIONS)+"gens_"+str(PROPOSAL_POPULATION_SIZE)+"_best_fitness_histories.png", best_fitness_histories, ver="proposal")
elif choice == "1":
    print(f"IGAシミュレーションの評価関数を選択\n0: ガウス関数\n1: スフィア関数\n2: ノイズ関数\n3: Rastrigin関数\n4: Ackley関数\n5: Schwefel関数")
    evaluate_num = input("評価関数の番号を入力してください: ")
    for i in range(5):
        print("普通のIGAシミュレーション"+str(i+1)+"回目を実行")
        best_fitness_histories.append(iga.run_simulation_normal_IGA(NUM_GENERATIONS=NUM_GENERATIONS, POPULATION_SIZE=POPULATION_SIZE, evaluate_num = int(evaluate_num), times = i+1))
        print("普通のIGAシミュレーション"+str(i+1)+"回目が完了")
    log_fitness_histories(int(evaluate_num), "_"+str(NUM_GENERATIONS)+"gens_"+str(POPULATION_SIZE)+"_best_fitness_histories.png", best_fitness_histories, ver="conventional")
elif choice == "3":
    print("音の確認を実行")
