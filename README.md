...
# 補間について
自分の研究の一番重要な点であり、新規性であり、コアの部分。いろいろな手法を考えた。
評価個体を選択するための事前評価を補間するときと適応度を補間するときとで意味合いが違う
### 事前評価を補間する場合
与えられている情報は前世代の評価済み個体であり、探索空間の形状を把握するようなもの。
この後の評価個体の選択で間違った地図が修正されるか正しい地図が広げられるかが手法によって違ったりする。
間違った地図というのは前世代で最適解付近に個体を評価できずに局所解付近の個体を評価した場合に意図せず山が生まれているときの地図である。
間違った地図を信用し続けると最適解に山がないという学習をしてしまうかもしれない。
### 適応度を補間する場合
与えられている情報は現世代の評価済み個体であり、事前評価とかで作った地図を基に当てはめていくような作業、ここで新たな値を発見しようみたいなのはない。
1. 距離
    - 補間に使うのはベスト・ワーストの2個体
    - ベストとワーストの適応度の間となる
    - 最大距離を事前に計算しておく
...
2. ガウス
    - 補間に使うのはベスト・ワーストの2個体
    - ベストとワーストからベストを頂上としワーストを一番裾野としたガウス関数を推定
    - 結構無理やりな方法
    - 悪くはなかったが、局所解に突っ走る方法でもある
    - まともにやると後半にベストとワーストがほぼ重なり補間できなくなる
3. RBF
    - 評価した個体すべてを学習データとして既定関数を作りそれを基に補間
    - kernelはスタンダード目なthin_plate_spline
    - ガタついた曲線を描くかもしれない
4. IDW
    - 評価した個体すべてを学習データとして既定関数を作りそれを基に補間
    - 評価個体の適応度は越えない
    - 評価した個体との距離の二乗の逆数が重みとなる
    - 近い点の適応度に近くなって2つの中間の距離の個体は2つの中間くらいの適応度になる
    - 割となだらかな曲線を描いて補間する
5. ハイブリッド
    - 評価個体をすべて使う上に過去の評価個体も学習データにする
    - IDWとRBF補間を組み合わせたもの
    - 前半(1~5)世代ではIDWでなだらかに補間
    - 後半(6~15)世代ではRBF補間を用いて一気に登るようなイメージ
...
- やること/やったこと
    ...
    - 12/22
        ...
        - 世代数を15に拡張
        - 過去の評価個体のデータをどんどん追加しすべて補間に使うことにした、速度的にも最大135個体しか貯まらないので申し分なし
        - また、事前評価の段階では評価済み個体から離れている個体程ボーナスを掛けるようにして探索に重きを置くようにした
        - この処理は前半世代の時はボーナス強めで後半ではボーナスを弱めるように係数を設定しているため、役割を分けれている
        - 結果としてはすべて9個体補間なしよりは性能が高いのだが、Ackley関数やガウス+コサイン関数だと伸びが悪い。
        - あとそもそも普通の評価関数だと、50個体補間なしの時点でほぼ性能が頭打ちになる。逆説的にAckley関数とガウス+コサイン関数が難しすぎる可能性もあり？
    - 12/23
        - 自分の研究がサロゲートモデルっちゅうのに分類されることを学んだ。
        - fill distance(被覆半径)とnn distance(最近傍半径)という概念を学習
        - 被覆半径は現世代の全個体視点でそれぞれの個体から最も近い評価済み個体との距離の内最も大きい距離
        - これが遠い内は、まだ探索できていない空間が広いということを表す。
    ...
    - 1/21
        - 先行研究の手法を試したら頻度は連続値をとるIGAでは意味がなかったことが分かった。これを実験結果と言って載せるのははばかられるため予備実験みたいな扱いにすべきかと思った。
        - 頻度をとっているため、定義域の端に個体が生成されたときに端が一番多いと誤学習してしまうようだ
        - うまいこと改善したが最終的な結果は補間なし9個体より低いという結果になった．これはやはり評価個体数の不足によるものだと考えられる．実際のIGA使用を想定した条件ではダメ見たいだ
        - 小数個体・小数世代での安定した探索性能が提案手法のアピールポイントになりそう

    -2/1
        - 距離項の重みを変えて実験した，予備実験のつもり
            - 0.1刻みで0から1までやったが，1.0以外の違い自体は大差なかった，0.3よりむしろ0.1か0.8とかの方が最近傍距離が一番小さくなる回数が多くなっていた．
            - 予備実験の結果各評価関数でのベストな重みの平均として0.45を提案手法の重みとして採用した．